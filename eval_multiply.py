...
def _extract_answer_from_ids(token_ids, tokenizer) -> str:
    ...
    answer_str = full_str.split('<eos>')[0].strip()
    return answer_str
...
def evaluate_on_loader(model, tokenizer, val_loader, num_samples_to_eval=200, batch_size=20, K=None):
    """
    ...
    for _ in pbar:
        ...
        for i in range(batch_input_ids.size(0)):
            ...
            if len(answer_indices[0]) == 0: continue
    ...
    return {"accuracy": accuracy, "correct": correct_predictions, "total": total_samples}
